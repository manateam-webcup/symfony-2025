(function ($) {
  ...
  if (getSwitchToggleID) {
    ...
    if (switchToggle) {
      ...
      if (savedTheme) {
        html.classList.add(
          savedTheme === "dark-mode" ? "dark-mode" : "light-mode"
        );
      }
  ...
  $(document).ready(function () {
    ...
    $("#toggleBootstrapDirection").click(function () {
      ...
      if (
        currentHref.includes("bootstrap.min.html") &&
...
function updateEmotion() {
  ...
  avatar.classList.add("morph");

  // Update image and text after a slight delay for a nicer effect
  setTimeout(() => {
    const emotion = emotions[index];
    avatar.src = emotion.image;
    text.textContent = emotion.text;

    // Reset morph effect after animation
    avatar.classList.remove("morph");
...
window.addEventListener('beforeinstallprompt', (e) => {
  ...
  if (!localStorage.getItem('pwaInstallPromptShown')) {
    ...
    const dismissBtn = document.getElementById('dismissCardBtn');
